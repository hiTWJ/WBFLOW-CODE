...
class WBsRGB:
  def __init__(self, gamut_mapping=2, upgraded=0):
    dir = '/home/lcx/wbsrgb/WB_sRGB_Python/models/'
    if upgraded == 1:
      self.features = np.load(dir+'features+.npy')  # encoded features
      self.mappingFuncs = np.load(dir+'mappingFuncs+.npy')  # correct funcs
      self.encoderWeights = np.load(dir+'encoderWeights+.npy')  # PCA matrix
      self.encoderBias = np.load(dir+'encoderBias+.npy')  # PCA bias
      self.K = 75  # K value for NN searching
    else:
      self.features = np.load(dir+'features.npy')  # encoded features
      self.mappingFuncs = np.load(dir+'mappingFuncs.npy')  # correction funcs
      self.encoderWeights = np.load(dir+'encoderWeights.npy')  # PCA matrix
      self.encoderBias = np.load(dir+'encoderBias.npy')  # PCA bias
  ...
  def correctImage(self, I):
    ...
    similar_Features = self.features[idH[:,0],:]
    distance = D_sq[idH[:,0],:]
