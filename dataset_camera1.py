...
class BasicDataset_(Dataset):
    def __init__(self, name, patch_size=128,cam_name=['Canon1DsMkIII'],patch_num_per_image=1, type='train'):
        ...
        self.input_cam, self.gt_cam, self.label_cam = [],[],[]
        for nn in cam_name:
            cam_idx = self.cam_list.index(nn)
            input_cam1,gt_cam1,label_cam1 = \
                self.ran_cam(self.input,self.gt,self.label,cam_idx+1)
            self.input_cam.extend(input_cam1)
            self.gt_cam.extend(gt_cam1)
            self.label_cam.extend(label_cam1)

        self.input_cam = np.array(self.input_cam)
        self.gt_cam = np.array(self.gt_cam)
        self.label_cam = np.array(self.label_cam)
    ...
    def ran_cam(self, input_, gt, label, cam_inx):
        inx_cam1 = np.argwhere(label == cam_inx)
        input_cam1 = input_[inx_cam1[:, 0]]
        gt_cam1 = gt[inx_cam1[:, 0]]
        label_cam1 = label[inx_cam1[:, 0]]
        return input_cam1,gt_cam1,label_cam1

    def __getitem__(self, i):

        cam_label = np.zeros((len(self.cam_list),1,1))
