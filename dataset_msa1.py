...
class BasicDataset(Dataset):
    ...
    @classmethod
    def preprocess(cls, pil_img, patch_size, patch_coords, flip_op, input_):
        ras = np.random.randint(3)
        if ras > 0 and input_==True:
            random_factor = np.random.randint(0, 31) / 10.  # 随机因子
            pil_img = ImageEnhance.Color(pil_img).enhance(random_factor)  # 调整图像的饱和度
        ...
        if flip_op == 1:
            ...
            # pil_img = pil_img[:,:,:3]
            pil_img = seq.augment_image(pil_img)
    ...
    def __getitem__(self, i):
        ...
        if self.type == 'train':
            ...
            in_img_patches = self.preprocess(in_img, self.patch_size, (patch_x, patch_y), flip_op,True)
            awb_img_patches = self.preprocess(awb_img, self.patch_size, (patch_x, patch_y), flip_op,False)
            for j in range(self.patch_num_per_image - 1):
                ...
                patch_x = np.random.randint(0, high=w - self.patch_size)
                patch_y = np.random.randint(0, high=h - self.patch_size)
                temp = self.preprocess(in_img, self.patch_size, (patch_x, patch_y), flip_op,True)
                temp = temp[:3, :, :]

                in_img_patches = np.append(in_img_patches, temp, axis=0)

                temp = self.preprocess(awb_img, self.patch_size, (patch_x, patch_y), flip_op,False)
                temp = temp[:3, :, :]
                awb_img_patches = np.append(awb_img_patches, temp, axis=0)

                # cam_label = np.append(cam_label, cam_label, axis=0)
