...
class Evaluator:
    def __init__(self, args, CAM, device):
        ...
        "best_model"
        checkpoint = torch.load('/home/lcx/artflow/output/model/'+self.type+'/'+'105000.tar')
        args.start_iter = checkpoint['iter']
        self.glow.load_state_dict(checkpoint['state_dict'])

        "加载backbone"
        # model_dict = self.glow.state_dict()
        # pretrained_dict = torch.load('/home/lcx/artflow/output/model/'+self.type+'/'+'best_model.tar')['state_dict']
        #
        # pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict}
        #
        # model_dict.update(pretrained_dict)
        # self.glow.load_state_dict(model_dict)
        ...
        "txt"
        self.error_txt = '/home/lcx/artflow/error/' + self.type+'_Deltae1.txt'
    ...
    def do_testing(self):
        ...
        with torch.no_grad():
            ...
            STD_avg = np.std(np.concatenate((F_avg_avg_can1[np.newaxis,:],F_avg_avg_can6[np.newaxis,:],F_avg_avg_fuj[np.newaxis,:],
                                             F_avg_avg_nik[np.newaxis,:],F_avg_avg_oly[np.newaxis,:],F_avg_avg_pan[np.newaxis,:],
                                             F_avg_avg_sam[np.newaxis,:],F_avg_avg_sony[np.newaxis,:])),axis=0)

            STD_std = np.std(np.concatenate(
                (F_std_avg_can1[np.newaxis, :], F_std_avg_can6[np.newaxis, :], F_std_avg_fuj[np.newaxis, :],
                 F_std_avg_nik[np.newaxis, :], F_std_avg_oly[np.newaxis, :],F_std_avg_pan[np.newaxis, :],
                 F_std_avg_sam[np.newaxis, :],F_std_avg_sony[np.newaxis, :])),axis=0)
...
def main():
    ...
    parser.add_argument('--train_cam',
                        default='GLOW_WB_RE_12000_15_32lflow', type=str, help='')
