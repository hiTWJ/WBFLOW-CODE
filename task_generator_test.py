...
def mini_camera_folders():
    "分成train和test两个fold list"
    data_folder = '/home/lcx/deep_final/deep_final/folds/'
    metatrain_folders = np.load(data_folder+'train_cam8_ct_5.npy',allow_pickle=True).item()
    metatest_folders = np.load(data_folder + 'val_cam8_ct_5.npy', allow_pickle=True).item()

    # random.seed(1)
    # random.shuffle(metatrain_folders['name'])
    # random.shuffle(metatest_folders['name'])

    return metatrain_folders,metatest_folders


class MiniCameraTask(object):
...
class MiniImagenetTask(object):
    ...
    def __init__(self, character_folders, num_classes, train_num,test_num):
        ...
        self.train_num = train_num
        self.test_num = test_num
        input = list(self.character_folders['name'])

        kk = random.sample(input,8)

        class_folders = random.sample(input,self.num_classes)
