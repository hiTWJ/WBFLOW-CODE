...
def val_camera(glow, test_loader,name,camera_name,i, tf_writer):
    ...
    # for i, (x_t, y_t, camera_t) in enumerate(test_loader):
      ...
      if (count+1) % 5 == 0:
        ...
        tf_writer.add_images(f'{name}/{camera_name}_{str(count)}/', pred_this.cpu().permute(0, 2, 3, 1).numpy()[0], i,dataformats="HWC")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_decay', type=float, default=5e-6) #5e-5
    "160000, 320000"
    ...
    parser.add_argument('--name', default='GLOW_WB_noconv_32_base', type=str, help='')
    ...
    # ---------------------------glow------------------------------
    from glow_wb import Glow,Camera_Glow_norev_re
    ...
    for total_it in range(args.start_iter, args.max_iter):
        ...
        if total_it == args.start_iter:
            with torch.no_grad():
                _ = glow.module(input_img,cam, forward=True)
                continue

        # (log_p, logdet, z_outs) = glow()
        z_c = glow(input_img,cam, forward=True)
        # z_wb = glow(gt_img, forward=True)
        # mean_wb,std_wb = calc_mean_std(z_wb)

        # reverse
        correct_img = glow(z_c,cam, forward=False)
        ...
        # T = 10.0
        # p_mean_wb, p_mean_learned = F.softmax(mean_wb / T, dim=1), F.softmax(learned_mean / T, dim=1)
        # p_std_wb, p_std_learned = F.softmax(std_wb / T, dim=1), F.softmax(learned_std / T, dim=1)
        #
        # # use KLD for consistency loss
        # loss_mean = F.kl_div(p_mean_learned.log(), p_mean_wb, reduction='batchmean')
        # loss_std = F.kl_div(p_std_learned.log(), p_std_wb, reduction='batchmean')
        #
        #
        # # loss_mean = mae_loss.compute(learned_mean,mean_wb)
        # # loss_std = mae_loss.compute(learned_std,std_wb)
        # loss_sta = (loss_mean+loss_std)/2

        # loss = loss_sta + loss_mae
        loss = loss_mae
